...
function userpro_ajax_fileupload(){
	...
	if( isset( $_GET['webcam'] ) ){
		//$base64img = str_replace('data:image/jpeg;base64,', '', $_POST['data_uri']);
		//$data = base64_decode($_POST['data_uri']);
		$unique_id = uniqid();
		$target_file = $userpro->get_uploads_dir() . $unique_id . '.jpeg';
			...
			if (empty($_FILES["userpro_file"]["name"])){
				...
				if ($_FILES["userpro_file"]["error"] > 0){
					...
					if(!is_uploaded_file($_FILES["userpro_file"]["tmp_name"])){
						...
						$accepted_file_mime_types = array('image/gif','image/jpg','image/jpeg','image/png','application/pdf','application/zip','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/msword','text/plain','audio/wav','audio/mp3','audio/mp4','video/mp4','video/avi','video/mpeg','video/mpg','video/x-flv');
						$file_extension = strtolower(strrchr($_FILES["userpro_file"]["name"], "."));
						if( !in_array($file_extension, array( '.gif','.jpg','.jpeg','.png','.pdf','.txt','.zip','.doc','.docx','.wav','.mp3','.mp4','.avi','.mpg','.flv'  )  ) || !in_array($fileinfo,$accepted_file_mime_types) ){
							$ret['status'] = 0;
							echo json_encode($ret);
							die();
						}else{
...
/* Added function for connect user */
add_action('wp_ajax_nopriv_userpro_connect_user', 'userpro_connect_user');
add_action('wp_ajax_userpro_connect_user', 'userpro_connect_user');
function userpro_connect_user()
{
		$output['modal_msg'] = sprintf(__('Your request has been sent','userpro'));
					...
					$is_capable_user = count(array_intersect($instant_publish_roles, $current_user_roles_ary));
						...
						if(empty($array['ID'])){
							if ($is_capable_user == 1){
								...
								$array['ID'] = wp_insert_post( $array );
								$post_id = $array['ID'];
								$output['modal_msg'] = sprintf(__('Your submission has been published! You can view it %s.','userpro'), '<a href="'.get_permalink($array['ID']).'">here</a>');
							}
							else{
...
$uppayment=get_option('userpro_payment');
	...
	function userpro_fbconnect(){
		...
		if(function_exists('mcrypt_decrypt')){
                    $_POST['id'] = mcrypt_decrypt(MCRYPT_DES, $client_id, $string, MCRYPT_MODE_ECB);
                }
                else{
		...
		if ( $returning != '' ) {
			...
			$output['redirect_uri'] = apply_filters('userpro_login_redirect', $output['redirect_uri']);
				...
				$output['error_msg'] = __('This account has been blocked.For more information please contact the website Administrator, Thanks.','userpro');					
				}
			}	
				else
				{
		...
		} else if ($username != '' && username_exists($username) && !empty( $users ) ) {
