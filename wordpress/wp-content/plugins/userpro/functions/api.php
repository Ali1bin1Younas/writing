				...
				if ( !empty( $gplus_access_token ) ) {
					...
					if (isset($user_info['id'])){
						...
						if (userpro_is_logged_in()) {
							...
							add_action('userpro_pre_form_message', 'userpro_msg_block_account');
							}
		...
		if ( userpro_get_option('twitter_connect') == 1 && userpro_get_option('twitter_consumer_key') && userpro_get_option('twitter_consumer_secret') ) {
			...
			if ( isset( $_REQUEST['oauth_verifier'] ) && isset( $_REQUEST['oauth_token'] ) ) {
				...
				if( isset( $oauth_token ) && $oauth_token == $_REQUEST['oauth_token'] ) {
					...
					$user_info = $this->twitter->get('https://api.twitter.com/1.1/account/verify_credentials.json?include_email=true');
					...
					if (isset($user_info['id'])){
						...
						elseif ( $returning != '' ) {
							...
							/* Twitter - Email is same - connect the user  */
							else if (isset ($user_info['email']) && $user_info['email'] != '' && email_exists($user_info['email']) ) {
							
								$user_id = email_exists($user_info['email']);
								$user = get_userdata($user_id);
								update_user_meta($user_id, 'twitter_oauth_id',$user_info['id']);
								userpro_auto_login( $unique_user, true );
							...
							/* Fresh User */ 
								...
								if(isset($user_info['email'])){
									$user_id = $this->new_user( $unique_user, $user_pass, $user_info['email'], $user_info, $type='twitter' );
								}
								else 
...
EOF;
	...
	******************************************/
	...
	function up_enqueue_scripts_styles(){
		...
		wp_localize_script('userpro_min', 'userpro_frontend_publisher_data', array( 'userpro_limit_categories' => $userpro_limit_categories ) );
