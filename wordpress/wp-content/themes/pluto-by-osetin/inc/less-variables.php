...
class my_less extends lessc {
  ...
  function __construct() {
    ...
    $this->color_schemes_arr = array('al_pacino', 'blue_sky', 'dark_night', 'black_and_white', 'pinkman', 'space', 'grey_clouds', 'almond_milk', 'clear_white', 'sakura', 'mighty_slate', 'retro_orange', 'nova');
    $this->load_less_vars();
  }

  public function get_var($default_key, $default_value){
  ...
  public function get_merged_var($key, $default = ''){
    if(isset($this->merged_vars[$key]))
      return $this->merged_vars[$key];
    else
      return $default;
  }

  public function get_merged_vars(){
    return $this->merged_vars;
  }
  ...
  function load_less_vars() {
    ...
    if (osetin_get_field('logo_type', 'option') == 'text'){
    ...
    if(osetin_get_field('no_limit_single_post_width', 'option') != true){
      if(osetin_get_field('single_post_maximum_width', 'option')){
        $vars[ 'singlePostMaxWidth' ] = $this->add_px(osetin_get_field('single_post_maximum_width', 'option'));
      }else{
        $vars[ 'singlePostMaxWidth' ] = '900px';
      }
    }else{
      $vars[ 'singlePostMaxWidth' ] = 'inherit';
    }
    ...
    if(osetin_get_field('enable_custom_colors', 'option') == true) {
      ...
      if(osetin_get_field('body_background_color', 'option')){
        if(osetin_get_field('body_background_style', 'option') == 'dark'){
      ...
      if( osetin_get_field('menu_background_style', 'option') == 'light' ){
      ...
      // MENU GRADIENT
      if( osetin_get_field('menu_background_color', 'option') ){
      ...
      // SUB TOP BAR
      $vars[ 'subBarBackgroundColor' ]        = $this->custom_or_default('sub_bar_background_color' , 'subBarBackgroundColor');
      $vars[ 'subBarBackgroundType' ]         = $this->custom_or_default('sub_bar_background_type' , 'subBarBackgroundType');

      // HIGHLIGHT ACCENTS
      if(osetin_get_field('top_body_highlight', 'option')){
        $vars['menuTopHighlightBorder']         = '4px solid '.osetin_get_field('top_body_highlight', 'option');
        $vars['contentTopHighlightBorder']      = '4px solid '.osetin_get_field('top_body_highlight', 'option');
      }else{
      ...
      if( osetin_get_field('sidebar_background_color', 'option') ){
        ...
        if( osetin_get_field('sidebar_background_style', 'option') == 'light' ){
      ...
      if( osetin_get_field('body_background_style', 'option') == 'light' ){
    ...
    $this->merged_vars = array_merge($this->default_vars, $vars);
  }
...
add_filter( 'less_vars', array($my_less, 'get_merged_vars'), 10, 2 );
